...
15. [Convention layer — `ConventionCatalog` / `StepNormalizer` / `ingenious_testcase_parameterize`](#15-convention-layer)

---

## 15. Convention layer

**Feature** — a single source of truth for INGenious authoring conventions,
consumed by every guidance and enforcement surface (76 tools total).

| File | Purpose |
|---|---|
| [Engine/src/main/java/com/ing/engine/mcp/ConventionCatalog.java](Engine/src/main/java/com/ing/engine/mcp/ConventionCatalog.java) | Machine-readable rules (E/W/I ids + severities), input-grammar helpers (`@literal`, `Sheet:Column`, `{Sheet:Column}`, `#id`, `%var%`), and two rendered docs: condensed `initialize.instructions` + full `ingenious://docs/conventions`. |
| [Engine/src/main/java/com/ing/engine/mcp/StepNormalizer.java](Engine/src/main/java/com/ing/engine/mcp/StepNormalizer.java) | One write path for all step-creating tools. Bare literals are deterministically `@`-prefixed (payload bodies excluded); convention violations surface as `warnings[]` in tool results. |

**Changes**

* `ingenious_testcase_create` / `add_step` / `insert_step` / `edit_step` now
  normalize inputs via `StepNormalizer` and return `warnings[]` (incl. the W1
  "externalise literals" nudge).
* `ingenious_testcase_validate` is now a full linter: Execute/reusable
  reference checks (E3), data-reference existence for `Sheet:Column` and
  `{Sheet:Column}` (E4), scenario-name collisions between TestPlan/ and
  ReusableComponents/ (E5), GlobalData `#id` in inputs (E6), `@`-prefixed
  objects (E7), OR page references (E2), plus warnings (fixed sleeps, missing
  assertions, secrets, MANUAL-marker shape, monolith test cases) and info
  (missing descriptions). Errors/warnings/info carry rule ids.
* **New tool `ingenious_testcase_parameterize`** — two-phase data-driving:
  `mode=scan` lists candidates (whole `@literal` inputs + individual JSON
  payload leaf paths with suggested sheet/column names); `mode=all` or
  `mode=selected` + `selections[]` applies. Values move into a data-sheet row
  keyed to the test case; inputs are rewritten as `Sheet:Column`, payload
  fields as embedded `{Sheet:Column}` tokens. Supports `dryRun`, `env`,
  per-selection `column`/`sheet`/`paths` overrides, reusables via
  `reusable=true`; idempotent (reuses the row on re-run).
* MCP `initialize` now returns `instructions` so every client (IDE chat, REPL,
  external MCP clients) automatically injects the conventions into model
  context.
* Resources: new `ingenious://docs/conventions`; `step-schema`,
  `best-practices` and `getting-started` corrected to the canonical grammar
  (the `@Data.column` / `${column}` notations were wrong and are gone).
* **XML payload parameterization** — `ingenious_testcase_parameterize` also
  parses XML bodies: element text and attributes become candidates with
  `/root/child[1]/@attr`-style paths and are rewritten to `{Sheet:Column}`
  tokens in place (JSON and XML formats are auto-detected per step).
* `ingenious_gen_testcase` gained `parameterize=true` (+ optional `sheet`) to
  chain generation and full parameterization in one call.
* Linter additions: W3 (interaction without a preceding `waitFor*`),
  W10 (identical step sequences across test cases → extract a reusable),
  I1 (generic scenario/test-case names).
* New prompts: `make_data_driven` (guided scan→apply parameterization) and
  `extract_reusable` (W10-driven reusable extraction with Execute recomposition).
* Conventions embedded in both AI front-ends: the `ingenious ai` REPL planner
  system prompt and the IDE chat `AuthoringSkill` system prompt now include
  `ConventionCatalog.condensedInstructions()`.
* Distributable AI assets under `Resources/ai/`: `INGenious.skill.md` (skill
  format), `copilot-instructions.md` (repo template) and a `Readme.md`
  explaining when they are needed (MCP-connected clients get the conventions
  automatically).
* Conformance test suite (`Engine/src/test/java/com/ing/engine/mcp/`):
  `ConventionCatalogTest`, `StepNormalizerTest`, `MCPToolSurfaceTest` — tool
  naming/schema conventions and the input grammar are now CI-enforced.
