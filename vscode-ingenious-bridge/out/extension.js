...
const DISCOVERY_FILE = path.join(os.homedir(), '.ingenious', 'bridge.json');
...
let outputChannel;
const LOG_FILE = path.join(os.homedir(), '.ingenious', 'bridge.log');
...
class SidebarProvider {
    ...
    // ========================================================================
    async updateHybridHistory(model) {
...
Be extremely concise. No conversational filler. Facts only.`;
    ...
    // Logging
    // ========================================================================
    addLogEntry(prompt, response, modelName) {
        const entry = { prompt, response, modelName };
        ...
        try {
            for await (const part of chatRequest.stream) {
                ...
                if (part instanceof vscode.LanguageModelTextPart) {
                    ...
                    log(`  Part ${fragmentCount}: type=${part.constructor?.name || typeof part}`);
                }
            }
        }
        catch (streamError) {
...
function openAiToolsToLm(tools) {
...
function isToolCallPart(part) {
    const ctor = vscode.LanguageModelToolCallPart;
...
function messagesToPayload(messages) {
    const payload = [];
    const ToolCallPart = vscode.LanguageModelToolCallPart;
    const ToolResultPart = vscode.LanguageModelToolResultPart;
    for (const msg of messages) {
        ...
        if (role === 'assistant') {
            ...
            if (ToolResultPart) {
                ...
                payload.push(vscode.LanguageModelChatMessage.User([
            ...
            if (text && text.trim().length > 0) {
                payload.push(vscode.LanguageModelChatMessage.User([new vscode.LanguageModelTextPart(text)]));
            }
        }
    }
    return payload;
...
async function handleChatCompletions(req, res) {
    ...
    try {
        ...
        log(`response: ${toolCalls.length} tool_calls${names ? ' [' + names + ']' : ''}, ` +
            `${responseText.length} text chars`);
        provider?.addLogEntry(extractMessageText(messages[messages.length - 1]?.content), toolCalls.length > 0
            ? '[tool_calls] ' + toolCalls.map(t => t.function.name).join(', ')
            : responseText, model.family);
        res.json({
            id,
...
function activate(context) {
    ...
    const webviewDisposable = vscode.window.registerWebviewViewProvider('ingenious-bridge-view', provider);
