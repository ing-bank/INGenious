...
public class ProjectRunner implements TestRunner {
    ...
    private void applyOverride(String key, String value) {
        if (key.startsWith("run.")) {
            ...
            getProject().getProjectSettings().getCapabilities()
                    .getOrCreateCapabiltiesFor(browser).update(capability, value);
        } else if (key.startsWith("db.")) {
            String args[] = key.split("\\.", 3);
            if (args.length < 3) return;
            String db = args[1];
            String property = args[2];
            getProject().getProjectSettings().getDatabaseSettings()
                    .getDBPropertiesFor(db).put(property, value);
        } else if (key.startsWith("context.")) {
            String args[] = key.split("\\.", 3);
            if (args.length < 3) return;
            String context = args[1];
            String property = args[2];
            getProject().getProjectSettings().getContextSettings()
                    .getContextOptionsFor(context).put(property, value);
            ...
            getProject().getProjectSettings().getKafkaSSLConfigurations()
            ...
            String api = args[1];
            String property = args[2];
            getProject().getProjectSettings().getDriverSettings()
                    .getAPIPropertiesFor(api).put(property, value);
        } else if (key.startsWith("lambdatest.")) {
            // §B-1: LambdaTest Grid Capabilities
            getProject().getProjectSettings().getLambdaTestCaps()
            ...
            String prop = args[2];
            getProject().getProjectSettings().getCapabilities()
                    .getOrCreateCapabiltiesFor(browser).update(prop, value);
            ...
            String name = args[1];
            String prop = args[2];
            com.ing.datalib.settings.emulators.Device d =
                    getProject().getProjectSettings().getDevices().getOrCreateDevice(name);
            if ("RemoteURL".equalsIgnoreCase(prop) || "Remote URL".equalsIgnoreCase(prop)) {
                ...
                getProject().getProjectSettings().getCapabilities()
                        .getOrCreateCapabiltiesFor(name).update(prop, value);
            }
            ...
            getProject().getProjectSettings().getTestMgmtModule()
                    .setOption(module, optName, value);
        }
    }
