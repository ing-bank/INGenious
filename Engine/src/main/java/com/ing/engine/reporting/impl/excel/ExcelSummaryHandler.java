...
public class ExcelSummaryHandler extends SummaryHandler implements PrimaryHandler {
    ...
    private void createExcel() throws Exception {
        if (!RunManager.getGlobalSettings().isTestRun()) {
            ...
            String case_check = Control
                .getCurrentProject()
                .getProjectSettings()
                .getExecSettings(current_release, current_testset)
                .getRunSettings()
            ...
            if (case_check != null && case_check.equalsIgnoreCase("true")) {
                ...
                String datajspath =
                    FilePath.getLatestResultsLocation() + File.separator + "data.js";
                ...
                try {
                    ...
                    String excelreport =
                        FilePath.getLatestResultsLocation() +
                        File.separator +
                        "SummaryExcelReport.xlsx";
                    String excelreport_tm =
                    ...
                    for (int i = 0; i < executions.size(); i++) {
                        ...
                        String sheetname =
                            sheetnumber.getAndIncrement() + "_" + eachexecution.get("testcaseName");
                        ...
                        cell.setCellValue(
                            eachexecution.get("scenarioName") +
                            "_" +
                            eachexecution.get("iterationType") +
                            "_" +
                            eachexecution.get("testcaseName") +
                            "_" +
                            eachexecution.get("browser")
                        );
                        ...
                        for (int j = 0; j < objectsarray.size(); j++) {
                            ...
                            roww1
                                .createCell(1)
                                .setCellValue(
                                    Integer.toString((int) objectsarray.get(j).get("stepno"))
                                );
                            roww1
                                .createCell(2)
                                .setCellValue((String) objectsarray.get(j).get("stepName"));
                            roww1
                                .createCell(3)
                                .setCellValue((String) objectsarray.get(j).get("action"));
                            roww1
                                .createCell(4)
                                .setCellValue((String) objectsarray.get(j).get("description"));
                            roww1
                                .createCell(5)
                                .setCellValue((String) objectsarray.get(j).get("status"));
                            roww1
                                .createCell(6)
                                .setCellValue((String) objectsarray.get(j).get("tStamp"));
                        }
                    ...
                    if (
                        cellvalue.getStringCellValue().equalsIgnoreCase("ReleaseName-Testsetname")
                    ) {
                    ...
                    workbook.close();
                    System.out.println(
                        "Latest Excel Report Path " + FilePath.getLatestResultsLocation()
                    );
    ...
    private void launchexcel() throws IOException {
        ...
        try {
            ...
            LOGGER.info(
                "Unable To Open Report, Please Check If Excel location is added to System Path"
            );
        }
    }
