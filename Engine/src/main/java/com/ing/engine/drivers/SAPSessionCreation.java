    ...
    public void launchSession(RunContext context) throws UnCaughtException {
        ...
        if (!isSAPBrowser(browserName)) {
            System.out.println(
                "[SAP] Browser '" +
                browserName +
                "' is not SAP-based. Skipping SAP session initialization."
            );
            session = null;
            SAPProcess = null;
            return;
        ...
        if (!isSAPEmulatorConfigured(browserName)) {
            System.out.println(
                "[SAP] SAP emulator '" +
                browserName +
                "' is not configured. Skipping SAP session initialization."
            );
            session = null;
            SAPProcess = null;
            return;
        ...
        try {
            SAPProcess =
                SAPSessionFactory.startSAPProcess(
                    context,
                    Control.getCurrentProject().getProjectSettings()
                );
            session =
                SAPSessionFactory.createSAPSession(
                    context,
                    Control.getCurrentProject().getProjectSettings()
                );
            ...
            throw new UnCaughtException(
                "[SAP Session Exception] Cannot initialize SAP session for '" +
                browserName +
                "': " +
                ex.getMessage()
            );
