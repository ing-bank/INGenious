...
public class ImportCommand implements Callable<Integer> {
    ...
    @Command(
        name = "postman",
        description = "Import a Postman collection as test cases or reusables"
    )
    ...
    @Command(
        name = "bruno",
        description = "Import a Bruno collection (file or directory) as test cases or reusables"
    )
    ...
    public static class PlaywrightSubCommand implements Callable<Integer> {
        ...
        @Parameters(
            index = "0",
            description = "Playwright recording (Java source emitted by codegen, .txt or .java)"
        )
        private File file;
        ...
        @Option(
            names = { "--scenario" },
            description = "Target scenario name (default: derived from file name)"
        )
        private String scenarioName;

        @Option(
            names = { "--testcase" },
            description = "Test case name (default: derived from file name)"
        )
        private String testCaseName;
        ...
        public Integer call() {
            ...
            if (file == null || !file.exists()) {
                cli.printError("Recording file not found.");
                return 1;
            }
            try {
                Project project = new Project(path);
                ...
                cli.printSuccess(
                    "Imported Playwright recording as test case: " +
                    r.scenarioName +
                    "/" +
                    r.testCaseName +
                    " (" +
     ...
     */
    private static int importCollection(
