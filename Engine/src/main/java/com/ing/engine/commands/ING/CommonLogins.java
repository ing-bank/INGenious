...
public class CommonLogins extends General {
    ...
    public void storeCurrentTimeinMillisby1000() {
        try {
            ...
            if (variableName.matches("%.*%")) {
                ...
                Report.updateTestLog(
                    Action,
                    "[" + currentTimeSeconds + "]" + " value stored",
                    Status.DONE
                );
            } else {
    ...
    private void setCookies(String domain) {
        ...
        cookies.add(new Cookie("spin", "dummydummydummydummy").setDomain(domain).setPath("/"));
        cookies.add(new Cookie("lang", "en-GB").setDomain(domain).setPath("/"));
        ...
        for (String setCookie : setCookies) {
            ...
            if (nv.length == 2) {
                cookies.add(new Cookie(nv[0], nv[1]).setDomain(domain).setPath("/"));
            }
        }
    ...
    private void injectLocalStorage(String ck, String issuer, String type, int loa)
        throws PlaywrightException, ForcedException, ActionException {
        {
            JsonArray data = JsonParser
                .parseString((String) response.get(key).body())
                .getAsJsonArray();
            ...
            token.addProperty(
                "accessToken",
                tokenData.get("access_token").getAsString().replace("\n", "")
            );
            ...
            String script = String.format(
                "localStorage.setItem('TokenManager.profiles', JSON.stringify([%s]));" +
