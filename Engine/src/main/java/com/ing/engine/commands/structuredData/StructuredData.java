...
public class StructuredData extends General {
    ...
    public void assertXmlPathResultNotContains() {
        try {
            ...
            String inputValue = getInputValue(Input);
            if (!value.contains(inputValue)) {
                Report.updateTestLog(Action, "Element text [" + value + "] does not contain [" + inputValue + "] as expected", Status.PASSNS);
            } else {
                Report.updateTestLog(Action, "Element text [" + value + "] contains [" + inputValue + "] but should not",
                        Status.FAILNS);
            }
        } catch (IOException | ParserConfigurationException | XPathExpressionException | DOMException
                | SAXException ex) {
    ...
    public void assertXmlPathResultEquals() {
        try {
            ...
            if (value.equals(inputValue)) {
                ...
                Report.updateTestLog(Action, "Element text is [" + value + "] but is expected to be [" + inputValue + "]", Status.FAILNS);
            }
        } catch (IOException | ParserConfigurationException | XPathExpressionException | DOMException
                | SAXException ex) {
            Logger.getLogger(this.getClass().getName()).log(Level.OFF, null, ex);
