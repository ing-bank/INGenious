...
public class StringOperations extends General {
    ...
    private String getVarValue(String strArg) {
        if (strArg.matches("%.*%")) return getVar(strArg); else if (
            strArg.matches("^\\{.*:.*\\}")
        ) return getDatasheet(strArg); else if (strArg.matches("\".*\"")) return strArg.substring(
            1,
            strArg.length() - 1
        ); else return "";
    }
     ...
     */
    @Action(
        object = ObjectType.STRINGOPERATIONS,
        desc = "Concats String inputs within testcase [<Data>]",
        input = InputType.YES,
    ...
    public void Concat() {
        if (!Condition.isBlank()) {
            ...
            for (String part : concatList) {
                ...
                if (count >= limit) {
                    Report.updateTestLog(
                        ...
                        "Input [" +
                        Data +
                        "] not added to variable " +
                        Condition +
                        ". String value exceeds expected limit: " +
                        limit +
                        ".",
                        Status.FAIL
                    );
                    ...
                    Report.updateTestLog(
                        ...
                        "] not added to variable " +
                        Condition +
                        ". String contains invalid input format.",
                        Status.FAIL
                    );
                    isInValidInput = !isInValidInput;
                    break;
            ...
            if (!isInValidInput && !concatStr.equals("")) {
                ...
                if (getVar(Condition) != null) Report.updateTestLog(
                    "Concat",
                    "Input [" +
                    Data +
                    "] concatenated. Output [" +
            ...
            Report.updateTestLog(
                ...
                "Input [" +
                Data +
                "] not added to variable. No variable name assigned to operation.",
                Status.FAIL
            );
        }
    }
    ...
    @Action(
        object = ObjectType.STRINGOPERATIONS,
        desc = "Trim white spaces of String input within testcase",
        input = InputType.YES,
        condition = InputType.YES
    )
    public void Trim() {
    ...
    public void Substring() {
        if (!Condition.isBlank()) {
            ...
            if (subStringList.size() == 2 || subStringList.size() == 3) {
                ...
                String s3 = subStringList.size() == 3
                    ? getVarValue(subStringList.get(2).trim())
                ...
                if (isNumeric(s2) && isNumeric(s3)) {
                    ...
                    Report.updateTestLog(
                        ...
                        "] not added to variable " +
                        Condition +
                        ". String parameter should be instance of a number.",
                        Status.FAIL
                    );
                    isInValidInput = !isInValidInput;
                }
            ...
            if (getReplaceList.size() == 3 || getReplaceList.size() == 4) {
                ...
                if (s.equals("")) {
                    ...
                    if (s4.equals("first") || s4.equals("all")) {
                        strNewReplace =
                            s4.equals("first") ? s.replaceFirst(s2, s3) : s.replaceAll(s2, s3);
                    } else {
                        Report.updateTestLog(
                            "Replace",
                            "Input [" +
                            ...
                            "] not added to variable " +
                            Condition +
                            ". Type of replace should be 'first' or 'all'.",
                            Status.FAIL
                        );
                        isInValidInput = !isInValidInput;
                    }
    ...
    @Action(
        object = ObjectType.STRINGOPERATIONS,
        desc = "Converts String input to lower case within testcase",
        input = InputType.YES,
        condition = InputType.YES
    )
    public void ToLower() {
        if (!Condition.isBlank()) {
            ...
            if (!isInValidInput && !strToLower.equals("")) {
                ...
                if (getVar(Condition) != null) Report.updateTestLog(
                    ...
                    "Input [" +
                    Data +
                    "] converted to lower case. New value [" +
                    strToLower +
                    "] stored into variable " +
                    Condition +
                    ".",
        ...
        if (!Condition.isBlank()) {
            ...
            if (!isInValidInput && !strToUpper.equals("")) {
                ...
                if (getVar(Condition) != null) Report.updateTestLog(
                    ...
                    "Input [" +
                    Data +
                    "] converted to upper case. New value [" +
                    strToUpper +
                    "] stored into variable " +
                    Condition +
                    ".",
    ...
    public void Split() {
        if (!Condition.isBlank()) {
            ...
            if (getSplitList.size() == 3 || getSplitList.size() == 4) {
                ...
                String s4 = getSplitList.size() == 4
                    ? getVarValue(getSplitList.get(3).trim())
                    : "-1";
                if (isNumeric(s3) && isNumeric(s4)) {
                    ...
                    if (s.equals("")) {
                        ...
                        strSplitted =
                            strSplittedArr.length - 1 >= index ? strSplittedArr[index] : "";
    ...
    @Action(
        object = ObjectType.STRINGOPERATIONS,
        desc = "Get occurence of String input within testcase",
        input = InputType.YES,
        condition = InputType.YES
    )
            ...
            if (getOccurenceList.size() == 2) {
                ...
                if (s2.length() == 1) {
                    ...
                    Report.updateTestLog(
                        ...
                        "] not added to variable " +
                        Condition +
                        ". String parameter should be single character to get occurence.",
                        Status.FAIL
                    );
                    isInValidInput = !isInValidInput;
                }
