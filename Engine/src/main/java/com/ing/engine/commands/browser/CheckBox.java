...
public class CheckBox extends Command {
            ...
            if (Locator.isEnabled()) {
                ...
                if (Locator.isChecked()) {
                    Report.updateTestLog(
                        "check",
                        "Checkbox '" + Locator + "'  has been selected/checked successfully",
                        Status.DONE
                    );
                } else {
                    Report.updateTestLog(
                        "check",
                        "Checkbox '" + Locator + "' couldn't be selected/checked",
                        Status.FAIL
                    );
                }
            } else {
                Report.updateTestLog(
                    "check",
                    "Checkbox '" + Locator + "' is not enabled",
                    Status.FAIL
                );
            }
        } else {
        ...
        if (Locator != null) {
            if (Locator.isEnabled()) {
                ...
                if (!Locator.isChecked()) {
                    Report.updateTestLog(
                        "uncheck",
                        "Checkbox '" + Locator + "'  has been un-checked successfully",
                        Status.DONE
                    );
                } else {
                    Report.updateTestLog(
                        "uncheck",
                        "Checkbox '" + Locator + "' couldn't be un-checked",
                        Status.FAIL
                    );
                }
            } else {
    ...
    public void SetChecked() {
        try {
            ...
            Report.updateTestLog(
                Action,
                "Setting checked status of" + "[" + ObjectName + "] as [" + Data + "]",
                Status.DONE
            );
