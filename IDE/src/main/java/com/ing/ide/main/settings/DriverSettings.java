...
//                getTotalBrowserList().toArray()));
    ...
    private List<String> getTotalDBList() {
        List<String> list = settings.getDBProperty().getDbList();
        return list;
    }

    private List<String> getTotalContextList() {
        List<String> list = settings.getContextOption().getContextList();
        return list;
    }
    ...
    private void checkAndLoadDatabases() {
        if (dbCombo.getSelectedIndex() != -1) {
            ...
            if (settings.getDBProperty().getDbList() != null) {
    ...
    private void checkAndLoadContexts() {
        if (contextCombo.getSelectedIndex() != -1) {
            ...
            if (settings.getContextOption().getContextList() != null) {
    ...
    private void loadDB(String dbName) {
        ...
        model.setRowCount(0);
        LinkedProperties prop = settings.getDBProperty().getDBPropertiesFor(dbName);
        if (prop != null) {
            for (Object key : prop.orderedKeys()) {
                Object value = prop.get(key);
                model.addRow(new Object[]{key, value});
    ...
    private void loadContext(String contextName) {
        ...
        model.setRowCount(0);
        LinkedProperties prop = settings.getContextOption().getContextOptionsFor(contextName);
        if (prop != null) {
            for (Object key : prop.orderedKeys()) {
                Object value = prop.get(key);
                model.addRow(new Object[]{key, value});
    ...
    private void saveDBProperties() {
        if (dbCombo.getSelectedIndex() != -1) {
            ...
            settings.getDBProperty().addDB(dbCombo.getSelectedItem().toString(), properties);
        }
    }
    ...
    private void saveContextProperties() {
        if (contextCombo.getSelectedIndex() != -1) {
            ...
            settings.getContextOption().addContext(contextCombo.getSelectedItem().toString(), properties);
        }
    }
...
//        }
       ...
       testConn.addActionListener(new java.awt.event.ActionListener() {
        ...
        contextPropTable.setModel(new javax.swing.table.DefaultTableModel(
                new Object[][]{
                        ...
                        {"userID", "<userID>"},
                        {"password", "<password>"},
                        {"storageStatePath", "<yourpath>"},
                        {"setGeolocation", "52.1326,5.2913"},
    ...
    private void addContextPropButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addPropButtonActionPerformed
        DefaultTableModel model = (DefaultTableModel) contextPropTable.getModel();
        model.addRow(new Object[]{});
    }

    private void removeContextPropButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_removePropButtonActionPerformed
    ...
    private void addNewDB() {
        ...
        if (!getTotalDBList().contains(newdbName)) {
            settings.getDBProperty().addDBName(newdbName);
            dbCombo.addItem(newdbName);
            dbCombo.setSelectedItem(newdbName);
            settings.getDBProperty().addDBProperty(newdbName);
            loadDB(newdbName);
        } else {
    ...
    private void deleteDB() {
        if (dbCombo.getSelectedIndex() != -1) {
            String dbName = dbCombo.getSelectedItem().toString();
            settings.getDBProperty().delete(dbName);
    ...
    private void addNewContext() {
        ...
        if (!newContextName.isBlank()) {
            if (!getTotalContextList().contains(newContextName) || getTotalContextList().isEmpty()) {
                settings.getContextOption().addContextName(newContextName);
                contextCombo.addItem(newContextName);
                contextCombo.setSelectedItem(newContextName);
                settings.getContextOption().addContextOptions(newContextName);
    ...
    private void deleteContext() {
        if (contextCombo.getSelectedIndex() != -1) {
            String contextName = contextCombo.getSelectedItem().toString();
            settings.getContextOption().delete(contextName);
