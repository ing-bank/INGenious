...
> **3.1.x refresh:** target INGenious 3.1.0 (`ingenious-api` 3.0, Playwright 1.58.0 per
> the root `pom.xml`). Deploy built plugins to `Dist/release/plugins/<plugin>/`. After
> deploy, confirm the plugin's actions are live by calling `ingenious_action_list` /
> `ingenious_action_search` (the same catalog the test-authoring skills validate against)
> instead of inspecting jars by hand.
