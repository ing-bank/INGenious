...
The most important thing to understand is the **DataProcessor pipeline** that runs before your action method. The Data column gets processed by `DataProcessor.resolve()` which resolves `%runtimeVars%`, `SheetName:Column` datasheet references, `=expressions`, `>JavaScript`, and `#globalData`. The `@` prefix is the escape hatch — it tells the processor "treat the rest as literal."

For clean parameterization, **`assertVariableFromDataSheet`** is the superior choice because it separates the variable reference (Condition column) from the expected value (Data column, which gets datasheet-resolved naturally). The single-column `assertVariable` is more limited — you can't easily mix a runtime variable on the left with a datasheet value on the right without the `@` prefix workaround.