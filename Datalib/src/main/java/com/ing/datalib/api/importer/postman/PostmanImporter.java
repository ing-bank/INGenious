...
public class PostmanImporter implements CollectionImporter {
    ...
    public boolean supports(File fileOrDir) {
        ...
        return name.endsWith(".postman_collection.json")
    ...
    private static File resolveCollectionInDir(File dir) {
        File[] direct = dir.listFiles((d, name) -> name.toLowerCase().endsWith(".postman_collection.json"));
    ...
    @Override
    public NormalizedCollection parse(File file, List<ImportWarning> warnings) throws ImportException {
        ...
        if (file.isDirectory()) {
            ...
            if (resolved == null) {
                throw new ImportException("No Postman collection (*.postman_collection.json) found in folder: "
                        + file.getAbsolutePath());
            }
            warnings.add(ImportWarning.info(file.getName(),
        ...
        if (file.getName().toLowerCase().endsWith(".postman_environment.json")) {
            try {
                ...
                NormalizedCollection envOnly = new NormalizedCollection(env.getName(), ImportSource.POSTMAN);
        ...
        try {
            ...
            throw new ImportException("Failed to read Postman collection JSON: " + e.getMessage(), e);
        }
        ...
        if (!knownSchema) {
            warnings.add(ImportWarning.warn("info.schema",
                    "Unrecognised Postman schema '" + schema + "' — import will continue best-effort."));
        }
        ...
        for (JsonNode v : root.path("variable")) {
            nc.getVariables().add(new NormalizedVariable(
                    v.path("key").asText(),
                    v.path("value").asText(""),
            ...
            if (envFiles != null) {
                for (File ef : envFiles) {
                    try {
                        ...
                        warnings.add(ImportWarning.info(ef.getName(),
                                "Imported environment '" + env.getName() + "' ("
                                        + env.getVariables().size() + " variables)."));
                    } catch (ImportException ex) {
                        warnings.add(ImportWarning.warn(ef.getName(),
    ...
    private void walkItems(JsonNode items, List<String> folderPath, AuthConfig inheritedAuth,
                           NormalizedCollection nc, List<ImportWarning> warnings) {
        ...
        for (JsonNode item : items) {
            if (item.has("item") && item.get("item").isArray()) {
                ...
                AuthConfig folderAuth = item.has("auth") ? parseAuth(item.path("auth"), warnings,
                        String.join("/", child) + ".auth") : inheritedAuth;
    ...
    private NormalizedRequest parseRequest(JsonNode item, List<String> folderPath,
                                           AuthConfig inheritedAuth, List<ImportWarning> warnings) {
    ...
    private String parseUrl(JsonNode urlNode, APIRequest req, List<ImportWarning> warnings, String loc) {
    ...
    private RequestBody parseBody(JsonNode bodyNode, List<ImportWarning> warnings, String loc) {
        ...
        switch (mode) {
            case "raw": {
                ...
                String lang = bodyNode.path("options").path("raw").path("language").asText("text");
            ...
            case "urlencoded": {
                ...
                for (JsonNode kv : bodyNode.path("urlencoded")) {
                    ...
                    b.getUrlEncodedData().add(new KeyValuePair(
            ...
            case "formdata": {
                ...
                for (JsonNode kv : bodyNode.path("formdata")) {
                    ...
                    if ("file".equalsIgnoreCase(type)) {
                        warnings.add(ImportWarning.warn(loc,
                                "form-data file field '" + kv.path("key").asText()
                                        + "' — file paths must be set manually."));
                    }
                    b.getFormData().add(new KeyValuePair(
                            ImportUtils.rewriteVariables(kv.path("key").asText()),
                            ImportUtils.rewriteVariables(kv.path("value").asText("")),
                            true));
                }
            ...
            case "graphql": {
                ...
                b.setRawContent(ImportUtils.rewriteVariables(
                        "{\"query\":" + jsonStr(q) + ",\"variables\":" + (v.isEmpty() ? "null" : v) + "}"));
                return b;
            }
            case "file": {
                RequestBody b = new RequestBody();
    ...
    private static RequestBody.RawFormat mapRawFormat(String lang) {
        ...
        switch (lang.toLowerCase(Locale.ROOT)) {
            case "json": return RequestBody.RawFormat.JSON;
            case "xml":  return RequestBody.RawFormat.XML;
            case "html": return RequestBody.RawFormat.HTML;
            case "javascript": return RequestBody.RawFormat.JAVASCRIPT;
    ...
    private AuthConfig parseAuth(JsonNode authNode, List<ImportWarning> warnings, String loc) {
        ...
        switch (type) {
            ...
            case "apikey": {
                ...
                return AuthConfig.apiKey(ImportUtils.rewriteVariables(key),
                        ImportUtils.rewriteVariables(value), locEnum);
            }
            default:
                warnings.add(ImportWarning.warn(loc,
    ...
    private void translateTestScript(String script, APIRequest req,
                                     List<ImportWarning> warnings, String loc) {
        ...
        while (m.find()) {
            warnings.add(ImportWarning.info(loc,
                    "Detected pm.environment.set('" + m.group(1) + "', ...) — converted to TODO comment; "
    ...
    public static NormalizedEnvironment parseEnvironment(File file) throws ImportException {
        try {
            ...
            for (JsonNode v : root.path("values")) {
                env.getVariables().add(new NormalizedVariable(
                        v.path("key").asText(),
                        v.path("value").asText(""),
                        "secret".equalsIgnoreCase(v.path("type").asText(""))));
            }
